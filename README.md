...
The router excludes disabled/unavailable models, unknown prices, insufficient context/capabilities, and candidates below `routing.minimumQuality` (default 0.90). It then minimizes normalized expected cost × `costWeight` (0.55) plus normalized latency × `latencyWeight` (0.45), with deterministic ID tie-breaking. Cost estimates use context bytes as a conservative input token proxy plus maximum output tokens. Large tasks penalize priors without `repo_scale`; medium tasks penalize missing `reasoning`. If no candidate qualifies, the run fails without weakening the quality threshold.

A local JSONL ledger records model attempts, requested/served IDs, feature buckets, executable outcomes, tokens, charged cost, elapsed model time, and escalation. Success estimates use `(prior × priorStrength + successes) / (priorStrength + attempts)`, with priorStrength 10. Matching task-kind/language/complexity observations increasingly outweigh priors; latency is similarly smoothed. Planner DAG validation is recorded separately as `DAG_VALIDATED`, never as executable verified success. Final run failure removes positive credit from worker successes when reading history. This conservative attribution can penalize a correct worker when another worker causes final failure.
...
Before every request, the existing concurrency-safe budget reserves the full UTF-8 input bound and maximum output at provider-enforced price ceilings. Unknown prices or ambiguous charged cost stop spending. Explicit pre-generation rejections (HTTP 400/404/429) can select an untried eligible model; ambiguous transport errors/timeouts/5xx remain fail-closed because billing is unknown. Malformed tool protocols with known response cost can also fall back. Stalls select an untried stronger tier or higher-quality candidate with a compact handoff. Attempts remain bounded by worker iterations, pool exhaustion, and run budgets.
...
Run-wide USD, token, iteration, and wall-time limits are complemented by per-model-stage limits. Before sending a request, a shared ledger reserves a conservative maximum using UTF-8 request bytes as an input-token upper bound, maximum output tokens, and configured OpenRouter provider price ceilings in USD per million tokens. Concurrent reservations count against the same budget. This may stop a run before its nominal budget is fully spent. Exact response `usage.cost` replaces the reservation; prices are not multiplied into a fabricated “actual” cost. Missing cost or an ambiguous transport failure stops new requests and marks accounting incomplete. Provider billing is authoritative; an API failure can leave an unknown charge, and provider-side price/accounting anomalies cannot be prevented locally.

Stage budgets are evaluated after each bounded call/check cycle; a final in-flight request may exceed a stage threshold. Run-wide reservations prevent knowingly scheduling above the configured run budget. Shell commands and model requests have timeouts; cleanup and draining can extend wall time beyond the scheduling deadline.
