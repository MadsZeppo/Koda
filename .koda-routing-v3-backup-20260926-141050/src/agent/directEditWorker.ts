...
export class DirectEditWorker implements CodingWorker {
  ...
  async run(input: CodingWorkerInput): Promise<CodingWorkerResult> {
    ...
    try {
      ...
      // Some providers materialize optional tool fields with neutral defaults
      // (createContent: "", delete: false). On an existing target an empty
      // createContent is therefore absence, not a conflicting create action.
      // A missing target may still intentionally create an empty file.
      const wantsCreate = source === undefined && suppliedCreate;
      const conflictingCreate = source !== undefined && suppliedCreate && args.createContent.length > 0;
